...
import { ArrowLeft, Cctv, User, Inbox, Radio, MapPin } from "lucide-react";
...
export default function ViolationsIndexPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const cctvItems: FeedItem[] = ((cctv.data ?? []) as CctvRow[]).map((r) => ({
        ...
        roadId: r.road_id,
        roadName: ROAD_BY_ID[r.road_id]?.name ?? r.road_id,
        source: r.camera_label ?? "CCTV",
        sub: `${r.vehicle_count ?? 0} vehicles seen`,
      ...
      const citizenItems: FeedItem[] = ((citizen.data ?? []) as CitizenReportRow[])
        ...
        .map((r) => ({
          ...
          roadId: r.snappedRoadId ?? "",
          roadName: ROAD_BY_ID[r.snappedRoadId ?? ""]?.name ?? r.snappedRoadId ?? "Unknown",
          source: "Citizen",
          sub: r.violationType,
    ...
    <div className="flex min-h-screen w-full flex-col">
      ...
      <header className="glass sticky top-0 z-20 flex items-center justify-between gap-3 border-b border-line px-5 py-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0 leading-none">
            <div className="eyebrow !text-primary">VIOLATIONS</div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-5 py-3">
        <div className="flex items-center gap-4 text-[12.5px]">
          <Stat n={flaggedCount} label="Flagged" accent />
          <Stat n={cctvCount} label="CCTV" />
          <Stat n={citizenCount} label="Citizen" />
        </div>
        <div className="flex items-center gap-1.5">
          {(["all", "cctv", "citizen"] as Source[]).map((s) => (
            <button
              ...
              className={`rounded-full px-3 py-1.5 text-[12px] font-semibold capitalize transition-colors focus-visible:outline-2 focus-visible:outline-primary ${
                source === s ? "bg-primary text-white" : "bg-surface-2 text-muted hover:text-ink"
              }`}
            >
              {s === "all" ? "All" : s === "cctv" ? "CCTV" : "Citizen"}
            </button>
          ))}
        </div>
      </div>
      ...
      <main className="scroll-quiet flex-1 overflow-y-auto p-5">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 px-6 py-24 text-center">
            ...
            <p className="text-[13px] leading-relaxed text-muted">No violations recorded yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {filtered.map((it) => {
              ...
              return (
                <Link
                  key={`${it.kind}-${it.id}`}
                  href={it.roadId ? `/violations/${it.roadId}` : "#"}
                  className="panel group overflow-hidden rounded-2xl transition-shadow hover:shadow-[0_8px_24px_rgba(0,0,0,0.18)]"
                >
                  ...
                  <div className="flex flex-col gap-0.5 px-2.5 py-2">
                    <div className="flex items-center gap-1 text-[11px] text-primary">
                      ...
                      <span className="truncate font-semibold">{it.roadName}</span>
                    </div>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="tnum shrink-0 text-[10.5px] text-faint">
                        {ago.key === "reports.justNow"
                          ? "just now"
                          : ago.key === "reports.minAgo"
                          ? `${ago.n}m ago`
                          : ago.key === "reports.hrAgo"
                          ? `${ago.n}h ago`
                          : `${ago.n}d ago`}
                      </span>
                    </div>
